...
function initMap() {
  ...
  if (mapElement) {
    ...
    const map = window.L.map("map").setView(paulistaCoords, 15)
    ...
    points.forEach((point, index) => {
      ...
      const marker = window.L.marker(point.coords, { icon: markerIcon }).addTo(map)
    ...
    const paulistaLine = window.L.polyline(
      [
